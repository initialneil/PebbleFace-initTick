...
void init_date_layer(Window *window) {
  ...
  s_month_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_LIGHT_16));
  s_date_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_LIGHT_16));
  s_weekday_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_REGULAR_18));

  // Create month TextLayer
  s_month_layer = text_layer_create(GRect(s_win_w - 51, s_center.y - 10, 50, 25));
  //s_month_layer = text_layer_create(GRect(s_win_w - 42, 0, 50, 25));
  ...
  s_date_layer = text_layer_create(GRect(s_win_w - 23, s_center.y - 10, 50, 25));
  //s_date_layer = text_layer_create(GRect(s_win_w - 16, 0, 50, 25));
